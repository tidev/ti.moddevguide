...
public class KrollDemoProxy extends LifeCycleProxy implements KrollProxyListener {
	...
	private void sendCancelEvent(String message, String title) {
		if (cancelCallback != null) {
			...
			// Fire an event directly to the specified listener (callback)
			cancelCallback.callAsync(getKrollObject(), event);
		}
	}
	...
	public void requestDataWithCallback() {
		...
		Log.e(LCAT, "[KROLLDEMO] requestData callback is disabled until 1.8.1!");

		// Object result = requestDataCallback.callSync(getKrollObject(), new HashMap());
		//
